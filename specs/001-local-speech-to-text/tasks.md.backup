...
- [ ] T001 Create Xcode macOS app project at `/workspace/SpeechToText.xcodeproj` with Swift 5.9+ and macOS 12.0+ deployment target
- [ ] T002 Create Swift Package Manager manifest at `/workspace/Package.swift` with FluidAudio SDK v0.9.0+ dependency
- [ ] T003 Create project directory structure: `/workspace/Sources/`, `/workspace/Tests/`, `/workspace/Resources/`
- [ ] T004 [P] Configure Xcode build settings for Apple Silicon (arm64) with Release and Debug configurations
- [ ] T005 [P] Add required entitlements in `/workspace/SpeechToText.entitlements` (microphone, accessibility, input-monitoring)
- [ ] T006 [P] Create SwiftLint configuration at `/workspace/.swiftlint.yml` for code quality enforcement
- [ ] T007 Resolve Swift Package Manager dependencies via `swift package resolve` to download FluidAudio SDK
...
- [ ] T008 [P] Create RecordingSession model at `/workspace/Sources/Models/RecordingSession.swift` with state machine (idle, recording, transcribing, inserting, completed, cancelled)
- [ ] T009 [P] Create UserSettings model at `/workspace/Sources/Models/UserSettings.swift` with nested config structs (hotkey, language, audio, UI, privacy, onboarding)
- [ ] T010 [P] Create LanguageModel model at `/workspace/Sources/Models/LanguageModel.swift` with download status enum and 25 supported languages
- [ ] T011 [P] Create UsageStatistics model at `/workspace/Sources/Models/UsageStatistics.swift` with privacy-preserving aggregations
- [ ] T012 [P] Create AudioBuffer model at `/workspace/Sources/Models/AudioBuffer.swift` for in-memory audio handling (16kHz mono)

### Core Services

- [ ] T013 Create FluidAudioService at `/workspace/Sources/Services/FluidAudioService.swift` as Swift actor with async transcribe() method using FluidAudio SDK
- [ ] T014 [P] Create PermissionService at `/workspace/Sources/Services/PermissionService.swift` with protocol-based interface for microphone, accessibility, and input monitoring checks
- [ ] T015 [P] Create SettingsService at `/workspace/Sources/Services/SettingsService.swift` wrapping UserDefaults with Codable persistence for UserSettings
- [ ] T016 [P] Create StatisticsService at `/workspace/Sources/Services/StatisticsService.swift` with SQLite database schema and privacy-preserving aggregation logic

### App Infrastructure

- [ ] T017 Create AppState at `/workspace/Sources/SpeechToTextApp/AppState.swift` as @Observable class with app-wide shared state (settings, statistics, current session)
- [ ] T018 Create main app entry point at `/workspace/Sources/SpeechToTextApp/SpeechToTextApp.swift` with @main attribute and MenuBarExtra scene
- [ ] T019 Create AppDelegate at `/workspace/Sources/SpeechToTextApp/AppDelegate.swift` for app lifecycle, menu bar management, and hotkey initialization
- [ ] T020 [P] Create Constants file at `/workspace/Sources/Utilities/Constants.swift` with app-wide constants (sample rates, thresholds, default values)
- [ ] T021 [P] Create Color+Theme extension at `/workspace/Sources/Utilities/Extensions/Color+Theme.swift` with Warm Minimalism color palette (amber accents, frosted glass)
...
- [ ] T022 [P] [US1] Create AudioCaptureService at `/workspace/Sources/Services/AudioCaptureService.swift` using AVAudioEngine for 16kHz mono capture with real-time level callbacks
- [ ] T023 [P] [US1] Create HotkeyService at `/workspace/Sources/Services/HotkeyService.swift` using Carbon Event Manager APIs for global hotkey registration (default ⌘⌃Space)
- [ ] T024 [P] [US1] Create TextInsertionService at `/workspace/Sources/Services/TextInsertionService.swift` using AXUIElement APIs to insert text at cursor position
- [ ] T025 [US1] Create RecordingViewModel at `/workspace/Sources/Views/RecordingViewModel.swift` as @Observable class coordinating audio capture, FluidAudio transcription, and text insertion
- [ ] T026 [US1] Create WaveformView component at `/workspace/Sources/Views/Components/WaveformView.swift` with real-time audio level visualization (30+ fps, canvas-based)
- [ ] T027 [US1] Create RecordingModal view at `/workspace/Sources/Views/RecordingModal.swift` with frosted glass (.ultraThinMaterial), waveform, and spring animations
- [ ] T028 [US1] Integrate HotkeyService with AppDelegate to show RecordingModal on ⌘⌃Space press
- [ ] T029 [US1] Implement silence detection in RecordingViewModel using FluidAudio VAD to auto-stop recording after 1.5 seconds
...
- [ ] T033 [P] [US2] Create PermissionCard component at `/workspace/Sources/Views/Components/PermissionCard.swift` with icon, title, description, and grant button
- [ ] T034 [P] [US2] Create OnboardingViewModel at `/workspace/Sources/Views/OnboardingViewModel.swift` as @Observable class managing onboarding state and permission flow
- [ ] T035 [US2] Create OnboardingView at `/workspace/Sources/Views/OnboardingView.swift` with multi-step flow (welcome, microphone, accessibility, demo, completion)
- [ ] T036 [US2] Implement microphone permission request step in OnboardingView using AVCaptureDevice.requestAccess(for: .audio)
- [ ] T037 [US2] Implement accessibility permission request step in OnboardingView with visual instructions to open System Settings
- [ ] T038 [US2] Create interactive demo step in OnboardingView allowing user to test hotkey and see sample transcription
- [ ] T039 [US2] Add skip option for each permission with warning about limited functionality
- [ ] T040 [US2] Update AppDelegate to show OnboardingView on first launch (check UserSettings.onboarding.completed flag)
- [ ] T041 [US2] Add deep link to System Settings for permission granting in PermissionCard component
- [ ] T042 [US2] Store onboarding completion state in UserSettings via SettingsService
...
- [ ] T043 [P] [US3] Create MenuBarViewModel at `/workspace/Sources/Views/MenuBarViewModel.swift` as @Observable class fetching daily statistics and handling menu actions
- [ ] T044 [US3] Create MenuBarView at `/workspace/Sources/Views/MenuBarView.swift` with quick stats display and menu options (Start Recording, Open Settings, Quit)
...
- [ ] T048 [US3] Fetch daily statistics in MenuBarViewModel from StatisticsService (words transcribed today, total sessions)
- [ ] T049 [US3] Add real-time stats updates in MenuBarView when statistics change using Combine or @Observable updates
